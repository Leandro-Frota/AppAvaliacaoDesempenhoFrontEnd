...
export default function ResumeQuestion({ employee }) {
  ...
  const sectonsWithoutJustification = [
  ...
  return (
    <>
      <div
        ref={printRef}
        className=" w-full flex flex-col border-2 bg-slate-50 border-gray-400 rounded p-4 "
      >
        ...
        <div>
          {Object.entries(sections).map(([sectionName, items]) => (
            <div
              key={sectionName}
              className="flex flex-col gap-2 border border-gray-600 p-2"
            >
              ...
              {Object.entries(items).map(([itemKey, item]) => (
                <div
                  key={itemKey}
                  className="flex flex-col gap-1 border border-gray-600 p-2"
                >
                  <p>
                    <strong>Item:</strong>
                    {itemKey}
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-lg font-semibold">
          Total de Pontos: {person.totalScore}
        </p>
        <p className="text-lg font-semibold">
          Data de Avaliação: {person.evaluationDate}
        </p>
        <p className="text-lg font-semibold">
          Avaliado por: {person.evaluator}
        </p>
        <p className="text-lg font-semibold">
          Data de Entrega: {person.deliveryDate}
        </p>
      </div>
    </>
  );
